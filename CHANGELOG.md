...
- **Source-modifying Treatment family (12 presets)** — all W2 siblings shipped: `fluid_warp` (mask-bounded velocity field), `fluid_warp_full` (unbounded variant), `ripple_lens` (concentric refraction rings), `edge_lens` (orbiting refraction bumps), `field_advect_source` (SDF-gradient advection), `zone_brighten` (luminance boost inside ZONE_WINDOW), `zone_lens` (UV warp at zone edge), `spotlights` (Gaussian luminance lift from drifting particles), `drift_pinholes` (particle-masked source visibility), `drift_brushstrokes` (motion-blurred brushstroke trails), `edge_sparks` (edge-spawning brief glows with lifetime fade), `collision_ripples` (GPU-resident bounce-and-ripple via per-particle state machine), `portal_warp` (ghost-through-the-room UV smear). All twelve picker-grouped under "Source-modifying" via the W2.12 separator. (PCleanup.1.2 / 2.1–2.11)
- **`palette_extract` zone-aware reimagining** — new `zone_mode` param (0=ignore default, 1=strict ZONE_WINDOW posterise, 2=dual quantisation with separate `outside_levels`). Default 0 is bit-identical to pre-8.3a output; zone modes only activate when the operator opts in. (PCleanup.8.3a)
- **`collage` kaleidoscope + mosaic modes** — new `mode` param (0=grid default, 1=kaleidoscope mirror-fold via |fract(uv×2)−0.5|×2, 2=mosaic per-tile region sampling from source with deterministic seed). Default 0 preserves the existing 2×2 grid for all projects. (PCleanup.8.3b)
- **`blur_mask` distance-driven radius mode** — new `radius_mode` param (0=edge-band default, 1=distance-driven: sharp at mask edge, blurry toward interior via smoothstep). A `distance_falloff` slider controls the gradient width in normalised units. Default 0 is bit-identical to pre-8.3c output. (PCleanup.8.3c)
- **Treatment particle SSBO infrastructure** — `src/render/treatment_particles.rs`: dedicated 16-byte std430 Particle layout (`pos / vel / age / _pad`) with double-buffered SSBOs at bind-group slot 7. Shared `new_with_shaders` helper lets sibling Treatments swap compute / fragment WGSL while reusing pipeline boilerplate. Particles seed deterministically per `LayerState.layer_id` so the visual is replay-stable. (PCleanup.2.4)
- **`v3` feature is default-on** — `cargo build` (no flags) compiles the v3 UI / state machine / Command pattern / undo. The v2 UI is still reachable for regression testing via `cargo build --no-default-features --features osc,midi,video`. CI exercises the v3 path. (PCleanup.8.1)
- **BPM modulator divisor preset selector** — `Modulator::Bpm` had no UI in v1; operators authored the `divisor` field as a raw `f32`. Replaced the "no UI in v1" stub with a 4-option preset combo (`1/2 (double-time)`, `1 (every beat)`, `2 (half-time)`, `4 (quarter-time)`) plus `amp` + `offset` sliders. Off-preset divisor values snap to the closest preset's label without corrupting stored bits.
...
- **`MoveBezierAnchor` undo round-trip is bit-exact** — apply propagated the anchor delta to its handles via float `+= delta`, but the reverse undid the same delta via subtraction. Float `+= / -=` doesn't round-trip exactly for f32, so the next mutation's `debug_assert!` about stale Reverse-storage fired on undo. Fixed by snapshotting pre-apply handle bits into the reverse mutation and restoring them directly on undo. Found by the v3 proptest harness while preparing the W8.1 flag flip. (PCleanup.bezier-handle-reverse)
