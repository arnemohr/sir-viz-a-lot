...
- [x] PCleanup.2.5a — `drift_pinholes` (sibling of `mask_constrained_drift`) — particle SSBO mask, fragment shader inverts spotlights' luminance lift
...
- [x] PCleanup.2.11 — `portal_warp` (sibling of `fx_zone_portal_drift`) — shared spotlights compute; fragment smears source UVs toward/away from each drifting particle; closes Phase 4 zone-compute deferral
